...
-zi::atclone-hook() {
    ...
    local rc=0
    [[ -n $atclone ]] && __zi::countdown atclone && {
        local ___oldcd=$PWD
...
-zi::atpull-e-hook() {
    ...
    __zi::countdown atpull && {
        local ___oldcd=$PWD
        (( ${+ICE[nocd]} == 0 )) && {
            () { setopt localoptions noautopushd; builtin cd -q "$dir"; }
        }
        __zi::at-eval "$atpull" "$ICE[atclone]"
        rc="$?"
        () { setopt localoptions noautopushd; builtin cd -q "$___oldcd"; };
    }
...
-zi::compile-plugin-hook() {
    ...
    if [[ -z $ICE[(i)(\!|)(sh|bash|ksh|csh)] ]] {
        () {
            ...
            if [[ $tpe == snippet ]] {
                __zi::compile-plugin "%$dir" ""
            } else {
                __zi::compile-plugin "$id_as" ""
            }
        }
    }
} # ]]]
...
-zi::configure-base-hook() {
    ...
    [[ $flags == *\#* || ! -f $dir/configure ]] &&
        __zi::configure-run-autoconf "$dir" $flags
    ...
    if [[ $#files -gt 0 && -z $aflags || $flags == [^a-z0]#c[^a-z0]# ]]; then
        if (( ${+commands[cmake]} )); then
            ...
            m {pre}Running {cmd}cmake{pre} ${${${${#files}:#0}:+for its found {file}CMakeLists.txt{pre} input file{…}}:-because {flag}c{pre} flag given}
           __zi::countdown cmake && (
    ...
    if [[ $#files -gt 0 && -z $aflags || $flags == [^a-z0]#m[^a-z0]# ]]; then
        if (( ${+commands[meson]} )); then
            m {pre}Running {cmd}meson setup{pre} ${${${${#files}:#0}:+for its found {file}meson.build{pre} input file}:-because {flag}m{pre}flag given}
            __zi::countdown meson\ setup && (
...
-zi::extract-hook() {
    ...
    __zi::extract plugin "$extract" "$dir"
} # ]]]
...
# FUNCTION: __zi::configure-run-autoconf [[[
...
__zi::configure-run-autoconf() {
    ...
    # Manual reproduction of autoreconf run
        ...
        q=1
        m {pre}Running {cmd}aclocal{pre}, {cmd}autoconf{pre} and {cmd}automake${${${(M)flags:#*\#*}:+$msg}:-$msg_}
        __zi::countdown aclocal,\ autoconf,\ automake && \
            (
                cd -q $dir
                rm -f aclocal.m4
                aclocal -I m4 --force
...
# FUNCTION: __zi::download-file-stdout [[[
...
__zi::download-file-stdout() {
    ...
    if (( restart )) {
        ...
        if (( ${+commands[curl]} )); then
            if [[ -n $progress ]]; then
                command curl --progress-bar -fSL "$url" 2> >(__zi::single-line >&2) || return 1
            else
        ...
        if type curl 2>/dev/null 1>&2; then
            ...
            __zi::download-file-stdout "$url" "1" "$progress"
            return $?
        fi
    }
...
# FUNCTION: __zi::download-snippet [[[
# Downloads snippet
...
__zi::download-snippet() {
            ...
            (
                ...
                if (( ${+ICE[svn]} )) {
                    if [[ $update = -u ]] {
                        ...
                        if ! __zi::mirror-using-svn "$url" "-t" "$dirname"; then
                        ...
                        if (( ZINIT[annex-multi-flag:pull-active] == 2 )) {
                            ...
                            __zi::mirror-using-svn "$url" "-u" "$dirname" || return 4
                        }
                    } else {
                        __zi::mirror-using-svn "$url" "" "$dirname" || return 4
                    }
                    ...
                    if (( !skip_dl )) {
                        if { ! __zi::download-file-stdout "$url" 0 1 >! "$dirname/$filename" } {
                            if { ! __zi::download-file-stdout "$url" 1 1 >! "$dirname/$filename" } {
    ...
    "${${(M)OPTS[opt_-q,--quiet]:#1}:+, skip the -q/--quiet option for more information}.{rst}"; retval=4; }
        ...
        if [[ ${${:-$local_dir/$dirname}%%/##} != ${ZINIT[SNIPPETS_DIR]} ]] {
            ...
            __zi::store-ices "$pfx" ICE url_rsvd "" "$save_url" "${+ICE[svn]}"
        } elif [[ -n $id_as ]] {
    ...
    [[ (0 = ${+ICE[nocompletions]} && ${ICE[as]} != null && ${+ICE[null]} -eq 0) || 0 != ${+ICE[completions]} ]] && \
        __zi::install-completions "%" "$local_dir/$dirname" 0
...
# FUNCTION: __zi::forget-completion [[[
...
# FUNCTION: __zi::get-cygwin-package [[[
__zi::get-cygwin-package() {
    ...
    local mlst="$(mktemp)"
    while (( retry -- )) {
        if ! __zi::download-file-stdout https://cygwin.com/mirrors.lst 0 > $mlst; then
            __zi::download-file-stdout https://cygwin.com/mirrors.lst 1 > $mlst
        fi
    ...
    retry=3
    while (( retry -- )) {
        if ! __zi::download-file-stdout ${mirror}x86_64/setup.bz2 0 1 > $setup.bz2; then
            __zi::download-file-stdout ${mirror}x86_64/setup.bz2 1 1 > $setup.bz2
        fi
    ...
    while (( retry -- )) {
        integer retval=0
        if ! __zi::download-file-stdout $url 0 1 > $outfile; then
            if ! __zi::download-file-stdout $url 1 1 > $outfile; then
...
# FUNCTION: __zi::get-latest-gh-r-url-part [[[
...
__zi::get-latest-gh-r-url-part () {
  ...
  if [[ -z $urlpart ]]; then
    ...
    if [[ -z $tag_version ]]; then
      ...
      tag_version="$( { __zi::download-file-stdout $releases_url || __zi::download-file-stdout $releases_url 1; } 2>/dev/null | command grep -m1 -o 'href=./'$user'/'$plugin'/releases/tag/[^"]\+' )"
  ...
  init_list=( ${(@f)"$( { __zi::download-file-stdout $url || __zi::download-file-stdout $url 1; } 2>/dev/null | command grep -i -o 'href=./'$user'/'$plugin'/releases/download/[^"]\+')"} )
...
# FUNCTION: __zi::get-package [[[
__zi::get-package() {
    __zi::jq-check || return 1
    ...
    } elif { ! __zi::download-file-stdout $URL 0 1 2>/dev/null > $tmpfile } {
        # retry
        command rm -f $tmpfile
        __zi::download-file-stdout $URL 1 1 2>/dev/null >1 $tmpfile
    }
    ...
    local -a profiles
    profiles=("${(@f)$(__zi::json-get-value "$pkgjson" "${json_ices} | keys[]")}") \
        || return 1
    ...
    local -A metadata
    __zi::json-to-array "$pkgjson" "$json_meta" metadata
    if [[ "$?" -ne 0 || -z "$metadata" ]] {
    ...
    __zi::json-to-array "$pkgjson" "$json_profile" ICE
        ...
        if [[ $required == (bgn|dl|monitor) ]]; then
            if [[ ( $required == bgn && -z ${(k)ZINIT_EXTS[(r)<-> z-annex-data: zinit-annex-bin-gem-node *]} ) || \
                ...
                +zinit-message '{error}requires the {apo}`{annex}'${namemap[$required]}'{apo}`' \
                    ...
                    "{nl}for instructions."
                (( ${#profiles[@]:#$profile} > 0 )) && \
                    +zinit-message "{nl}Other available profiles are: {profile}${(pj:$pro_sep:)${profiles[@]:#$profile}}{rst}."
    ...
    if (( !${+ICE[git]} && !${+ICE[from]} )) {
        (
            local -A jsondata
            local URL=$(__zi::json-get-value "$pkgjson" "_resolved")
            local fname="${${URL%%\?*}:t}"
            ...
            if { ! __zi::download-file-stdout "$URL" 0 1 >! "$fname" } {
                if { ! __zi::download-file-stdout "$URL" 1 1 >! "$fname" } {
                    command rm -f "$fname"
...
# FUNCTION: __zi::install-completions [[[
...
__zi::install-completions() {
    ...
    __zi::any-colorify-as-uspl2 "$user" "$plugin"
    local abbrev_pspec=$REPLY

    __zi::exists-physically-message "$id_as" "" || return 1
    ...
    for c in "${completions[@]:A}"; do
        ...
        if [[ ( -z ${already_symlinked[(r)*/$cfile]} || $reinstall = 1 ) &&
            ...
            __zi::forget-completion "$cfile" "$quiet"
        else
            SKIPPED_COMPS+=( $cfile )
    ...
    __zi::compinit 1 1 &>/dev/null
} # ]]]
# FUNCTION: __zi::jq-check [[[
...
# FUNCTION: __zi::mirror-using-svn [[[
...
# FUNCTION: __zi::setup-plugin-dir [[[
...
__zi::setup-plugin-dir() {
    ...
    if __zi::get-object-path plugin "$id_as" && [[ -z $update ]] {
    ...
    (
        if [[ $site = */releases ]] {
            local tag_version=${ICE[ver]}
            if [[ -z $tag_version ]]; then
                tag_version="$({__zi::download-file-stdout $site/latest || __zi::download-file-stdout $site/latest 1;} 2>/dev/null | command grep -i -m 1 -o 'href=./'$user'/'$plugin'/releases/tag/[^"]\+')"
                tag_version=${tag_version##*/}
            fi
            local url=$site/expanded_assets/$tag_version

            __zi::get-latest-gh-r-url-part "$user" "$plugin" "$url" || return $?
            ...
            (
                ...
                for REPLY ( $reply ) {
                    ...
                    if { ! __zi::download-file-stdout "$url" 0 1 >! "${REPLY:t}" } {
                        if { ! __zi::download-file-stdout "$url" 1 1 >! "${REPLY:t}" } {
                    ...
                    if __zi::download-file-stdout "$url.sig" 2>/dev/null >! "${REPLY:t}.sig"; then
                        :
                    else
            ...
            (
                () { setopt localoptions noautopushd; builtin cd -q "$local_path"; } || return 1
                __zi::get-cygwin-package "$remote_url_path" || return 1
        ...
        if [[ $update != -u ]] {
            ...
            __zi::store-ices "$local_path/._zinit" ICE "" "" "" ""
            reply=(
                ${(on)ZINIT_EXTS2[(I)zinit hook:\!atclone-pre <->]}
                ${(on)ZINIT_EXTS[(I)z-annex hook:\!atclone-<-> <->]}
                ${(on)ZINIT_EXTS2[(I)zinit hook:\!atclone-post <->]}
    ...
    OPTS[opt_-q,--quiet]=1
    [[ (0 = ${+ICE[nocompletions]} && ${ICE[as]} != null && ${+ICE[null]} -eq 0) || 0 != ${+ICE[completions]} ]] && \
        __zi::install-completions "$id_as" "" "0"
...
__zi::update-snippet() {
    ...
    if { ! __zi::get-object-path snippet "$id_as" } {
    ...
    __zi::download-snippet "$save_url" "$url" "$id_as" "$local_dir" "$dirname" "$filename" "-u"
...
ziextract () {
    ...
    zparseopts -D -E -move=opt_move -move2=opt_move2 -norm=opt_norm -auto=opt_auto -nobkp=opt_nobkp || {
        +zinit-message "{info}[{pre}ziextract{info}]{error} Incorrect options given to" "\`{pre}ziextract{msg2}' (available are: {meta}--auto{msg2}," "{meta}--move{msg2}, {meta}--move2{msg2}, {meta}--norm{msg2}," "{meta}--nobkp{msg2}).{rst}"
        return 1
    }
    local file="$1" ext="$2"
    ...
    then
        ...
        then
            ...
            do
                ...
                then
                    (( !OPTS[opt_-q,--quiet] )) && +zinit-message "{info}[{pre}ziextract{info}]{msg2} detected a {meta}$type{rst} archive in the file {file}$fname{rst}."
                    ...
                    then
                        ...
                        do
                            ...
                            then
                                ...
                                (( !OPTS[opt_-q,--quiet] )) && +zinit-message "{info}[{pre}ziextract{info}]{msg2} detected a {obj}${type2}{rst} archive in the file {file}${fname}{rst}."
    ...
    __zi::extract-wrapper () {
        ...
        (( !OPTS[opt_-q,--quiet] )) && +zinit-message "{info}[{pre}ziextract{info}]{rst} Unpacking the files from: \`{obj}$file{msg}'{…}{rst}"
        $fun
        retval=$?
        if (( retval == 0 ))
        then
    ...
    →zinit-check () {
        (( ${+commands[$1]} )) || +zinit-message "{info}[{pre}ziextract{info}]{error} Error:{msg} No command {data}$1{msg}, it is required to unpack {file}$2{rst}."
    }
    case "${${ext:+.$ext}:-$file}" in
        ...
        ((#i)*.tar.bz2|(#i)*.tbz|(#i)*.tbz2) →zinit-extract () {
                →zinit-check bzip2 "$file" || return 1
                command bzip2 -dc "$file" | command tar -xf -
            } ;;
        ((#i)*.tar.gz|(#i)*.tgz) →zinit-extract () {
                →zinit-check gzip "$file" || return 1
                command gzip -dc "$file" | command tar -xf -
            } ;;
        ((#i)*.tar.xz|(#i)*.txz) →zinit-extract () {
                →zinit-check xz "$file" || return 1
                command xz -dc "$file" | command tar -xf -
            } ;;
        ((#i)*.tar.7z|(#i)*.t7z) →zinit-extract () {
                →zinit-check 7z "$file" || return 1
                command 7z x -so "$file" | command tar -xf -
            } ;;
        ((#i)*.tar) →zinit-extract () {
                →zinit-check tar "$file" || return 1
                command tar -xf "$file"
            } ;;
        ((#i)*.gz|(#i)*.gzip) if [[ $file != (#i)*.gz ]]
            then
        ...
        ((#i)*.7z|(#i)*.7-zip) →zinit-extract () {
                →zinit-check 7z "$file" || return 1
                command 7z x "$file" > /dev/null
            } ;;
        ((#i)*.dmg) →zinit-extract () {
                local prog
                for prog in hdiutil cp
                do
                ...
                if (( retval ))
                then
                    +zinit-message "{info}[{pre}ziextract{info}]{error} Error:{msg} problem occurred when attempted to copy the files" "from the mounted image: \`{obj}${file}{msg}'.{rst}"
                fi
                return $retval
            } ;;
        ((#i)*.deb) →zinit-extract () {
    ...
    unfunction -- __zi::extract-wrapper
    local -a execs
    ...
    then
        ...
        then
            ...
            then
                ...
                if (( ${#execs} > 7 ))
                then
                    +zinit-message "{info}[{pre}ziextract{info}]{rst} Successfully" "extracted and marked executable the appropriate files" "({obj}${(pj:$sep:)${(@)execs[1,5]:t}},…{rst}) contained" "in \`{file}$file{rst}'. All the extracted" "{obj}${#execs}{rst} executables are" "available in the {msg2}INSTALLED_EXECS{rst}" "array."
                else
                    +zinit-message "{info}[{pre}ziextract{info}]{rst} Successfully" "extracted and marked {obj}${#execs}{rst} executable the appropriate files" "({obj}${(pj:$sep:)${execs[@]:t}}{rst}) contained" "in \`{file}$file{rst}'."
                fi
            fi
        fi
    elif (( warning ))
