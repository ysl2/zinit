...
# FUNCTION: __zi::any-colorify-as-uspl2 [[[
...
# FUNCTION: __zi::compute-ice [[[
# Computes ICE array by priority
#   - input
#   - static
#   - saved
# 
...
__zi::compute-ice() {
  ...
  if [[ -d "$___s_path" || -d "$___path" ]]; then
    ...
    # plugin
    __zi::any-to-user-plugin "$___URL" ""
    ...
    __zi::exists-physically-message "$___user" "$___plugin" || return 1
  fi

  [[ $___pack = pack* ]] && (( ${#ICE} > 0 )) && __zi::pack-ice "${___user-$___URL}" "$___plugin"
...
# FUNCTION: __zi::exists-physically [[[
...
# FUNCTION: __zi::exists-physically-message [[[
...
__zi::exists-physically-message() {
  ...
  if ! __zi::exists-physically "$1" "$2"; then
    __zi::any-to-user-plugin "$1" "$2"
    if [[ $reply[1] = % ]]; then
      __zi::any-to-pid "$1" "$2"
      local spec1=$REPLY
...
__zi::first() {
  ...
  __zi::any-to-pid "$1" "$2"
  __zi::get-object-path plugin "$REPLY"
  integer ret=$?
  local dname="$REPLY"
  ...
  if [[ -e "$dname/$plugin.plugin.zsh" ]]; then
    ...
    __zi::find-other-matches "$dname" "$plugin"
  fi
...
__zi::two-paths() {
  ...
  __zi::get-object-path snippet "$url1"
  local_dirA=$reply[-3] dirnameA=$reply[-2]
  [[ -d "$local_dirA/$dirnameA/.svn" ]] && {
    svn_dirA=".svn"
    if { __zi::first % "$local_dirA/$dirnameA"; } {
      fileB_there=( ${reply[-1]} )
    }
  }

  __zi::get-object-path snippet "$url2"
